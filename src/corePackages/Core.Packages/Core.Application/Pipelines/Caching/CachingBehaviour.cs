    ...
    where TRequest : IRequest<TResponse>, ICachableRequest
{
    ...
    public CachingBehaviour(IDistributedCache cache, IConfiguration configuration, ILogger<CachingBehaviour<TRequest, TResponse>> logger)
    {
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        if (cachedResponse != null)
        {
            ...
            _logger.LogInformation($"Fetcehd from Cache -> {request.CacheKey}");
        }
        else
        ...
        )
    {
        ...
        await _cache.SetAsync(request.CacheKey, serializedData, cacheOptions);
